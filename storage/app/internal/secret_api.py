...
from app.utils.jwt_utils import get_current_user, get_admin_user, UserInfo
...
def get_all_secrets(
    ...
    return service.get_secrets_by_user_id(int(current_user.user_id))
...
def create_secret(
    ...
    secret_data.user_id = int(current_user.user_id)
...
@router.get("/admin/all", response_model=List[SecretResponse])
def get_all_secrets_admin(
    db: Session = Depends(get_db),
    admin_user: UserInfo = Depends(get_admin_user)
):
    """Get all secrets across all users (admin only)"""
    service = SecretService(db)
...
@router.get("/admin/user/{user_id}", response_model=List[SecretResponse])
def get_user_secrets_admin(
    user_id: int,
    db: Session = Depends(get_db),
    admin_user: UserInfo = Depends(get_admin_user)
):
    ...
    service = SecretService(db)
    return service.get_secrets_by_user_id(user_id)


@router.delete("/admin/{secret_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_secret_admin(
    secret_id: uuid.UUID,
    db: Session = Depends(get_db),
    admin_user: UserInfo = Depends(get_admin_user)
):
